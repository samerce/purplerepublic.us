...
export const Root = styled.div`
  position: relative;
  display: ${p => (p.isInMenu && p.isSelected)? 'none' : 'inline-block'};
  vertical-align: top;
  height: 80px;
  ...
  @media(max-width: 1220px) {
    display: ${p => p.canHide? p.isInMenu? p.isSelected? 'none' : 'block' : 'none' : 'inline-block'};
    width: ${p => p.canHide? p.isInMenu? '100%' : p.isSelected? '' : '100%' : ''};
  }
